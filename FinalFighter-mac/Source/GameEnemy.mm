...
- (id)init
{
    ...
    tanksInRange = [[NSMutableArray arrayWithCapacity:10] retain];
    ...
    bodyDef2.userData = [GameUserData userDataWithObject:self doTick:NO];
    bodyDef2.linearDamping = 0;
    bodyDef2.angularDamping = 0;
    turretSensorBody = world->CreateBody(&bodyDef2);
...
- (void)tick:(ccTime)dt
{
    ...
    if (actionDelay <= 0) {
        ...
        if (!leftSensor && !rightSensor && moveUp) {
    ...
    if (targetTank) {
        ...
        fire = NO;
        //moveTurretTo = sprite.rotation;
    }
}

- (void)contact:(GameObject *)aObject
{
    [super contact:aObject];
}
...
- (void)checkMine
{
    ...
    float r1 = CC_DEGREES_TO_RADIANS(sprite.rotation);
...
- (void)resetWithStartCoords:(GameStartCoords *)c
{
    ...
    turretSprite.rotation = sprite.rotation;
    turretShadowSprite.rotation = sprite.rotation;
    moveTurretTo = sprite.rotation;
}
